...
int32_t main(void)
{
    ...
    ///< 启动I2C数据传输
    I2C_SetFunc(M0P_I2C0,I2cAck_En);                   //ACK打开
    I2C_SlaveReadData(M0P_I2C0,u8Recdata,&u32RecvLen); //等主机写数据
    I2C_SlaveWriteData(M0P_I2C0,u8Recdata,&u32SendLen);//等主机读数据
    while(1)
    {
        ;
    }
...
void App_I2cCfg(void)
{
    ...
    I2C_SetFunc(M0P_I2C0,I2cHlm_En);            ///< 快速模式或高速模式（大于100kHz时）必须高速使能
